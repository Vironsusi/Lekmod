...
{
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::GenerateMeleeCombatInfo
	int iAttackerMaxHP = kAttacker.GetMaxHitPoints();
	int iDefenderMaxHP = pkDefender ? pkDefender->GetMaxHitPoints() : 0;
#else
...
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::GenerateMeleeCombatInfo - Attacking a City
		if (iAttackerTotalDamageInflicted >= iMaxCityHP && iDefenderTotalDamageInflicted >= kAttacker.GetMaxHitPoints())
		{
	...
	else
	{
...
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::GenerateMeleeCombatInfo - Attacking a Unit
		// Will both units be killed by this? :o If so, take drastic corrective measures
...
#ifndef NQM_PRUNING
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::GenerateMeleeCombatInfo - Attacking a Unit
		int iAttackerEffectiveStrength = iAttackerStrength * (pkDefender->GetMaxHitPoints() - range(kAttacker.getDamage(), 0, pkDefender->GetMaxHitPoints() - 1)) / pkDefender->GetMaxHitPoints();
		iAttackerEffectiveStrength = iAttackerEffectiveStrength > 0 ? iAttackerEffectiveStrength : 1;
		int iDefenderEffectiveStrength = iDefenderStrength * (kAttacker.GetMaxHitPoints() - range(pkDefender->getDamage(), 0, kAttacker.GetMaxHitPoints() - 1)) / kAttacker.GetMaxHitPoints();
		...
		else if (iAttackerTotalDamageInflicted >= pkDefender->GetMaxHitPoints() && kAttacker.IsCaptureDefeatedEnemy() && kAttacker.AreUnitsOfSameType(*pkDefender))
		{
...
void CvUnitCombat::ResolveMeleeCombat(const CvCombatInfo& kCombatInfo, uint uiParentEventID)
{
	        ...
	        pkDefender->IsCanDefend()) 		// Did the defender actually defend?
	{
		...
		//One Hit
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::ResolveMeleeCombat
...
#else
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::GenerateRangedCombatInfo
		...
		if(!pkTargetPlot->isCity())
		{
			...
			if(pkDefender)
			{
				...
				if(pkAttacker)
				{
					...
					if (bTargetDied)
#else
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::ResolveRangedUnitVsCombat
					// Defender Died
					if (iDamage + pkDefender->getDamage() >= pkDefender->GetMaxHitPoints())
#else
					if (iDamage + pkDefender->getDamage() >= GC.getMAX_HIT_POINTS())
...
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::ResolveRangedCityVsUnitCombat
					if (iDamage + pkDefender->getDamage() >= pkDefender->GetMaxHitPoints())
#else
					if (iDamage + pkDefender->getDamage() >= GC.getMAX_HIT_POINTS())
#endif
...
void CvUnitCombat::GenerateAirCombatInfo(CvUnit& kAttacker, CvUnit* pkDefender, CvPlot& plot, CvCombatInfo* pkCombatInfo)
{
...
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::GenerateAirCombatInfo - Attacker
...
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::GenerateAirCombatInfo - Defender
	...
	else
	{
		...
		{
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::GenerateAirCombatInfo - Calculate defense damage
			iDefenderDamageInflicted = kAttacker.GetMaxHitPoints() - kAttacker.getDamage();
#else
			iDefenderDamageInflicted = GC.getMAX_HIT_POINTS() - kAttacker.getDamage();
#endif
	...
	if(pkTargetPlot)
	{
		if(!pkTargetPlot->isCity())
		{
			...
			if(pkDefender)
			{
				...
				{
					//One Hit
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::ResolveAirUnitVsCombat
					if (pkDefender->GetCurrHitPoints() == pkDefender->GetMaxHitPoints() && pkAttacker->isHuman() && !GC.getGame().isGameMultiPlayer())
#else
					if (pkDefender->GetCurrHitPoints() == GC.getMAX_HIT_POINTS() && pkAttacker->isHuman() && !GC.getGame().isGameMultiPlayer())
#endif
...
{
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::GenerateAirSweepCombatInfo
	int iAttackerMaxHP = kAttacker.GetMaxHitPoints();
	int iDefenderMaxHP = pkDefender ? pkDefender->GetMaxHitPoints() : 0;
#else
	int iMaxHP = GC.getMAX_HIT_POINTS();
...
void CvUnitCombat::ResolveAirSweep(const CvCombatInfo& kCombatInfo, uint uiParentEventID)
{
	...
	if(pkDefender)
	{
		...
		{
			//One Hit
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::ResolveAirSweep
			if (pkDefender->GetCurrHitPoints() == pkDefender->GetMaxHitPoints() && pkAttacker->isHuman() && !GC.getGame().isGameMultiPlayer())
#else
			if (pkDefender->GetCurrHitPoints() == GC.getMAX_HIT_POINTS() && pkAttacker->isHuman() && !GC.getGame().isGameMultiPlayer())
#endif
					...
					if(pLoopUnit != NULL)
					{
						if(pLoopUnit != pkAttacker)
						{
							if(!pLoopUnit->isNukeImmune() && !pLoopUnit->isDelayedDeath())
							{
								...
								{
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::GenerateNuclearExplosionDamage
...
//		kAttacker.setMadeAttack(true);   /* EFB: Doesn't work, causes tactical AI to not dequeue this attack; but we've decided you don't lose your attack anyway */
	...
	else if(!pDefender->IsCanDefend())
	{
		...
		// Kill them!
#ifdef LOUP_UNIT_MAX_HP //CvUnitCombat::Attack
